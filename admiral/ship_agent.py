...
# 各探索モードの割合
MODE_PROB = np.array([3, 5, 2, 1, 2, 3])
# 和が1になるように
    ...
    def decide_move(self, ship_map, tank_map: np.ndarray):
        ...
        if random.random() < 0.1:
            ...
            if self.mode == MODE_WEIGHTED_NEAR:
                ...
                self.move = self.decide_near(tank_map)
            elif self.mode == MODE_NEAR_BIGGEST:
                self.move = self.decide_biggest_near(tank_map)
            elif self.mode == MODE_RANDOM:
                self.move = self.decide_random()
            elif self.mode == MODE_ESCAPE_4DIR:
                self.move = self.decide_escape(ship_map, tank_map)
            elif self.mode == MODE_WEIGHTED_4DIR:
                self.move = self.decide_weighted_4dir(ship_map, tank_map)
            else:
    ...
    def decide_weighted_4dir(self, ship_map, tank_map):
        ...
        for quad in range(0, 4):
            ...
            for y, x in zip(y_index, x_index):
                ...
                dir_point[quad] = dir_point[quad] + (4 - 4 * ((dx + dy) / 256)) * tank_map[y][x]
        dir = dir_point.argmax()
        return [Commands[dir], Commands[dir]]
